...
- **Origin (`https://ams.klappy.dev`)** — which AMS instance hosts the conversation. Discovery is solved trivially: the URL is the address. The reference deployment fronts one Worker behind multiple host CNAMEs (v1: `ams.klappy.dev` and `ams.truthkit.ai`), and magic links are portable across them — see [`canon/decisions/D0011-multi-host-cname-deployment`](./canon/decisions/D0011-multi-host-cname-deployment.md).
