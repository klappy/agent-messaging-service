...
complements: "D0016-buffering-and-persistence-as-wrapper-primitive (parallel wrapper-tier expansion), D0028-deterministic-identity-and-stream-resumability (the deterministic anon-account derivation that makes this route's transient synthesis stable), D0029-magic-link-as-ams-join-argument-on-mcp (the parallel auth path for consumers with stable connector URLs), canon/constraints/mcp-wrapper-conformance-for-conversational-ai (this decision motivates additive surface)"
...
A bearer that is presented but invalid (malformed, unknown, or revoked) is **not** silently downgraded to the transient path. The wrapper distinguishes "no bearer presented" from "bearer presented but invalid" by inspecting the `Authorization` header directly: an invalid bearer surfaces the underlying transport-level auth error so the caller learns their credential was rejected rather than having their actions attributed to a fresh anonymous identity. The transient-account fallback fires only when the caller did not present a bearer at all — which is the only case where the caller's intent is unambiguous.

The transient account has shape `account_id = acc_anon_<26-char-derived-suffix>`, `namespace = prebind.ns`, and is **not** persisted to KV — it lives only for the lifetime of the MCP session at the wrapper tier. Per `D0028` (deterministic identity and stream resumability), the suffix is derived from a peppered hash of the magic link's permissive token, **not** minted fresh per request. The same magic link therefore yields the same `acc_anon_*` identity across all requests, all reconnects, and all MCP transport sessions — which makes `D0019`'s `(account_id, conversation_id)` SessionDO keying actually work for the transient/anonymous case. Two anonymous peers attaching to the same conversation are distinguished by `stream_name` (the per-peer participation discriminator), not by `account_id` (the link-derived identity layer). Cross-session identity recognition for transient peers therefore works by design: same magic link → same SessionDO → buffer continuity across the natural lifecycle of stateless MCP consumers. Persistent ownership (cross-conversation, cross-link) remains opt-in via Door 2.
...
This closes the bootstrap-test step-4 gap (*"mint an account if it doesn't have one"*) for third-party MCP runtimes that cannot present custom bearers — ChatGPT's MCP "Apps" surface and claude.ai's web Connectors UI both support OAuth-only flows today, with no UX to paste a static bearer. Operators of those runtimes can paste only the magic link; the wrapper synthesizes the account on their behalf, scoped to the conversation already named in the URL. Header-capable runtimes (Claude Desktop, Claude Code, Cursor, Anthropic Messages API) continue to use Door 2 as before; the feature is additive on the magic-link route, not a relaxation of the `/mcp` route. Per `D0029`, Path C is **also** reachable on the bare `/mcp` endpoint via the `ams_join({ magic_link })` tool argument — the consumer flow ChatGPT-class clients with stable connector URLs actually follow. Both paths converge on the same deterministic transient-account synthesis per `D0028`. See `journal/2026-05-08-third-party-mcp-bearer-bootstrap-gap.tsv` and `journal/2026-05-08-path-c-completion-peer-identity-stream-stability.tsv` for the live tests and design conversations that produced this layered resolution.
