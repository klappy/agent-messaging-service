...
epoch: E0008.3
date: 2026-05-03
supersedes: "ams://canon/decisions/D0013-state-totals-via-snapshot-worker"
...
> The AMS Worker carries no telemetry code. A Tail Worker bound to it observes every invocation and writes the hook-layer rows D0010 names. State totals (accounts, conversations, live streams) are computed from sums of activity-stream events at query time in the dashboard's SQL, not by a scheduled poll. Three components, one dataset, zero scheduled jobs. The brittleness sources of D0013 — per-request inline hooks and per-minute Cron — are eliminated by replacing each with a natural CF-platform flow.

## Description

D0010 commits AMS observability to a two-layer pattern: a subscriber layer for wire-visible events and a hook layer for broker-internal events. D0013 implemented the hook layer as inline `writeDataPoint()` calls in the Worker and the state-totals problem as a per-minute Cron snapshot worker. Both choices were brittle in different ways:
...
The result is fewer moving parts, no scheduled jobs, no inline telemetry code in the broker, and a single SQL surface that answers both activity and state questions. D0014 keeps the schema D0013 specified (8 blobs + 6 doubles per `observability-payload-boundary`) and inherits D0013's slot-reuse pattern.
...
**Why this is more vodka than D0013's inline hooks:**
...
**Why this is more vodka than D0013's snapshot worker:**
...
D0014 has one fidelity loss compared to D0013, and it is named explicitly here so future readers can decide whether it deserves remediation.
...
If any of those fire, the remediation is **not** to bring back D0013's per-minute Cron. The remediation is one of:
...
- `ams://canon/decisions/D0013-state-totals-via-snapshot-worker` (superseded) — the prior approach this decision replaces; documents the rejected per-request-hook alternative.
...
- `ams://canon/principles/vodka-architecture-applied` — the four-question test this decision passes more cleanly than D0013.
