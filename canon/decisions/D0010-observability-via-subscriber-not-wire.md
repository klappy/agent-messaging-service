...
- The reference deployment cannot present observability data as a built-in dashboard route on the broker hosts (v1: `ams.klappy.dev`, `ams.truthkit.ai` per [`D0011-multi-host-cname-deployment`](./D0011-multi-host-cname-deployment.md)). Dashboards run as separate services that consume the hook sink and the subscriber sink, not as broker features.
