...
- **Generalized invocation contract**: the endpoint is generalized across personas. The request body carries `{persona_uri, pr_number, github_token?}`; the Worker enforces that `persona_uri` is on an allow-list of canon-resident personas (currently `[ams://canon/personas/ams-canon-code-auditor]`), preventing OIDC-authenticated callers from pointing at an attacker-controlled persona doc. Adding a new audit type means writing a new persona canon doc, adding the URI to the allow-list, and writing a thin caller workflow — no substrate changes. The reusable workflow is `.github/workflows/audit-gate-runtime-probe.yml`; the canon-code-sync caller is `.github/workflows/canon-code-sync-audit-runtime-probe.yml`.
