...
- **Recommended deployment**: configure rotation cadence ≥ buffer TTL. Frames older than the buffer TTL have already been evicted before the key under which they were emitted retires. Most of a key's emitted corpus is gone from the buffer by the time the key retires, leaving only at most one TTL-window of retired-key frames awaiting normal eviction. `D0016`'s primitive shape is unchanged; "buffer-cannot-outlive-its-key" is approached through configuration discipline, not through modifying the primitive.
- **Permitted but not recommended**: configure rotation cadence < buffer TTL. Every frame emitted under the retiring key is still in the buffer at retirement time (none has had time to age out under TTL); the buffer is populated mostly with retired-key frames until normal TTL/size eviction catches up. Consumers reading those frames see decryption failures; the buffer itself remains pure (it does not know what keys are or whether they have retired). The "buffer cannot outlive its key" property degrades — but the degradation is a consumer-wrapper concern (decryption fails), not a buffer concern.
